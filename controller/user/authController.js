...
const register = async (req, res) => {
    try {
        ...
        if (emailResult.success) {
            ...
            newUser.resetTokenExpiry = Date.now() + 30 * 60 * 1000; //30 mins
            await newUser.save();
        } else {
            return res.json({
                success: false,
...
const login = async (req, res) => {
    try {
        ...
        console.log('req.body=', req.body, 'email =', email, 'pass=', password);
...
const verifyOTP = async (req, res) => {
    try {
        ...
        res.cookie(
            'token',
            { token, loggedIn: true },
            { secure: true, maxAge, httpOnly: true }
        );
...
const logOutUser = async (req, res) => {
    Object.keys(req.cookies).forEach((key) => {
        res.clearCookie(key); // clear all cookies
    });
    return res
        .status(200)
